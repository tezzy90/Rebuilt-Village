...
exports.stripeWebhook = functions.onRequest({
    ...
    try {
        ...
        stripeEvent = stripe.webhooks.constructEvent(req.rawBody ?? req.body, sig, process.env.STRIPE_WEBHOOK_SECRET ?? '');
    }
    catch (err) {
    ...
    try {
        switch (stripeEvent.type) {
            case 'checkout.session.completed': {
                ...
                const fundType = (session.metadata?.fund_type ?? 'general');
                ...
                if (fundType === 'restricted' && projectId && amountTotal > 0) {
                    try {
                        await firebase_1.db.collection('donor_projects').doc(projectId).set({
                            raised: firebase_1.FieldValue.increment(amountTotal),
                            donationCount: firebase_1.FieldValue.increment(1),
                            lastDonationAt: firebase_1.FieldValue.serverTimestamp(),
